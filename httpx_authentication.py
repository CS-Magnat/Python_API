...
response_me = httpx.post("http://localhost:8000/api/v1/authentication/login", json=login_response)
qwe = response_me.json()

head = {
    "Authorization": f'Bearer {qwe["token"]["accessToken"]}'
}

get_req = httpx.get("http://localhost:8000/api/v1/users/me", headers=head)